...
14. [Resource Cleanup](#aws-resource-cleanup)
...
2. Configure:
   ...
   - CPU: `0.5 vCPU`
   - Memory: `1 GB`
   - **Task execution role:** `ecsTaskExecutionRole` ← select this (created in Phase 7)
...
> ⭐ **This is the most critical phase.** All four roles below must be created correctly or the pipeline will fail with permission errors.

---

### Role 1: `ecsTaskExecutionRole` (ECS Task Execution Role)

> ⚠️ This role is **required** for ECS Fargate to pull your Docker image from ECR and send logs to CloudWatch. Without it, containers will fail to start even after a successful build.

**Create the role:**

1. Go to **IAM** → **Roles** → **Create role**
2. Trusted entity type: **AWS Service**
3. Use case: Search **Elastic Container Service** → Select **Elastic Container Service Task**
4. Click **Next**
...
| `AmazonEC2ContainerRegistryFullAccess` | Full ECR read/write access |
| `CloudWatchLogsFullAccess` | Full CloudWatch log access |

5. Role name: `ecsTaskExecutionRole`
6. Click **Create role**

**Now add a Full-Access Inline Policy:**

1. Open `ecsTaskExecutionRole`
...
5. Policy name: `ecsTaskExecutionInlinePolicy`
6. Click **Create policy**

✅ `ecsTaskExecutionRole` is ready.

---

### Role 2: `CodeBuildECRRole` (CodeBuild Service Role)

> ⚠️ This is the role CodeBuild **assumes during every build**. It needs S3 access to download the source artifact from CodePipeline, ECR access to push images, and CloudWatch access to write logs. Missing S3 permissions is the most common cause of `AccessDenied` errors at the DOWNLOAD_SOURCE phase.

**Create the role:**

1. Go to **IAM** → **Roles** → **Create role**
2. Trusted entity type: **AWS Service**
3. Use case: Search **CodeBuild** → Select it
4. Click **Next**
...
| `AmazonS3FullAccess` | Download source artifact + upload build artifact from/to CodePipeline S3 bucket |
| `CloudWatchLogsFullAccess` | Write build logs to CloudWatch |
| `AmazonECS_FullAccess` | Access ECS resources during build |
...
5. Policy name: `CodeBuildFullInlinePolicy`
6. Click **Create policy**

✅ `CodeBuildECRRole` is ready.

---

### Role 3: `CodePipelineECSRole` (CodePipeline Service Role)

> ⚠️ CodePipeline does not appear in the standard AWS Service dropdown. Use the **Custom trust policy** method. This role also needs `codestar-connections:UseConnection` to connect to GitHub (Version 2).
...
4. Click **Next**
5. Skip managed policies → Click **Next** again
...
5. Policy name: `CodePipelineCustomPolicy`
6. Click **Create policy**

✅ `CodePipelineECSRole` is ready with `CodePipelineCustomPolicy` attached.
...
| --------- | ------- | --------------- |
| `ecsTaskExecutionRole` | ECS Fargate (pulling & running containers) | ECR pull, CloudWatch logs, Secrets Manager |
| `CodeBuildECRRole` | CodeBuild (during builds) | S3 read/write, ECR push, CloudWatch logs |
| `CodePipelineECSRole` | CodePipeline (orchestrating stages) | S3, CodeBuild, ECS, GitHub connection |

> ⭐ **All three roles must exist before proceeding to Phase 8.**
...
- Service role: **Existing service role** → `CodeBuildECRRole`
...
3. Configure:
   ...
   - Service role: Select existing role → `CodePipelineECSRole`
4. Click **Next**
...
### ❌ Error: `AccessDenied` on S3 / `DOWNLOAD_SOURCE` phase fails

**Cause:** `CodeBuildECRRole` is missing S3 permissions. CodeBuild downloads the source artifact from the CodePipeline S3 bucket and needs `s3:GetObject` on it.

**Fix:** Confirm the `CodeBuildFullInlinePolicy` inline policy is attached to `CodeBuildECRRole` with `s3:*`. Go to IAM → Roles → `CodeBuildECRRole` → Permissions tab and verify it.
...
**Fix:** Use **Custom trust policy** when creating the role (see Phase 7 - Role 3) and paste this JSON:
...
**Cause:** Usually a missing `ecsTaskExecutionRole` or wrong image URI.

**Fix checklist:**

- `ecsTaskExecutionRole` exists and is selected in the Task Definition
...
- `ecsTaskExecutionRole` has `AmazonECSTaskExecutionRolePolicy` and `AmazonEC2ContainerRegistryFullAccess` attached

---

### ❌ GitHub connection fails / Source stage fails

**Cause:** `CodePipelineECSRole` is missing `codestar-connections:UseConnection`. GitHub (Version 2) uses an AWS CodeStar Connection under the hood.

**Fix:** Confirm `CodePipelineCustomPolicy` includes both `codestar-connections:*` and `codeconnections:*` (AWS is migrating to the newer `codeconnections` namespace). Both are included in the policy from Phase 7.
...
└──────────┬──────────────────┬───────────────────┬────────┘
      ...
      (artifact          (role:              (role:
       in S3)         CodeBuildECRRole)   ecsTaskExecutionRole)
                           │
                           ▼
                    ┌─────────────┐
                    │  AWS ECR    │
...
| CodeBuild Inline Policy    | `CodeBuildFullInlinePolicy`|
| ECS Execution Inline Policy| `ecsTaskExecutionInlinePolicy` |
...
**Managed policies:** `AmazonECSTaskExecutionRolePolicy`, `AmazonEC2ContainerRegistryFullAccess`, `CloudWatchLogsFullAccess`

**Inline policy (`ecsTaskExecutionInlinePolicy`):**
...
```json
{
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Action": ["ecr:*", "logs:*", "s3:*", "secretsmanager:*", "ssm:*", "kms:*"],
      "Resource": "*"
    }
  ]
}
...
**Managed policies:** `AmazonEC2ContainerRegistryFullAccess`, `AmazonS3FullAccess`, `CloudWatchLogsFullAccess`, `AmazonECS_FullAccess`

**Inline policy (`CodeBuildFullInlinePolicy`):**
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Action": [
        "ecr:*", "s3:*", "logs:*", "codebuild:*", "ecs:*",
        "iam:PassRole", "secretsmanager:*", "ssm:*",
...
### `CodePipelineECSRole` — Trust Policy
...
**Inline policy (`CodePipelineCustomPolicy`):**
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Action": [
        "codepipeline:*", "codebuild:*", "ecs:*", "s3:*",
        "iam:PassRole", "ecr:*", "logs:*",
        "codestar-connections:*", "codeconnections:*",
        "codestar:*", "cloudwatch:*", "sns:*",
        "lambda:*", "cloudformation:*", "secretsmanager:*", "ssm:*"
      ],
      "Resource": "*"
    }
  ]
...
Delete all roles created for this project.
...
This role has an inline policy (`CodePipelineCustomPolicy`) attached directly. Deleting the role automatically removes the inline policy.

1. Search for `CodePipelineECSRole`
2. Click the role
3. Click **Delete** (top right)
4. Type the role name to confirm → Click **Delete**

### Delete `ecsTaskExecutionRole` (only if created fresh for this project)
...
```
IAM → Roles → Search CodeBuildECRRole, CodePipelineECSRole, ecsTaskExecutionRole → Should be gone
```
